...
def test_distribute_no_receivers(actual_fee_collector,
    ...
    assert post_distribution_caller_balance == pytest.approx(pre_distribution_caller_balance + effective_caller_fee)
    assert post_distribution_collector_balance == 0
    assert post_distribution_distributor_balance == pytest.approx(pre_distribution_collector_balance + pre_distribution_distributor_balance - effective_caller_fee)


def test_distribute_single_receiver(actual_fee_collector,
                                    actual_fee_distributor,
                                    global_fee_splitter,
                                    actual_crvusd,
                                    admin,
    ...
    assert post_distribution_caller_balance == pytest.approx(pre_distribution_caller_balance + effective_caller_fee)
    assert post_distribution_collector_balance == 0
    assert post_distribution_receiver_balance == pytest.approx(pre_distribution_receiver_balance + receiver_amount)
    assert post_distribution_distributor_balance == pytest.approx(
        pre_distribution_distributor_balance + distributor_amount)


def test_distribute_multiple_receivers(actual_fee_collector,
                                       actual_fee_distributor,
                                       global_fee_splitter,
                                       actual_crvusd,
                                       admin,
    ...
    pre_distribution_caller_balance = actual_crvusd.balanceOf(admin)
    pre_distribution_distributor_balance = actual_crvusd.balanceOf(actual_fee_distributor)
    pre_distribution_receiver_balances = [actual_crvusd.balanceOf(receiver) for receiver in receivers_to_use]
    ...
    post_distribution_caller_balance = actual_crvusd.balanceOf(admin)
    post_distribution_distributor_balance = actual_crvusd.balanceOf(actual_fee_distributor)
    post_distribution_receiver_balances = [actual_crvusd.balanceOf(receiver) for receiver in receivers_to_use]
